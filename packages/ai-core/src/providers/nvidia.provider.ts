...
export class NvidiaProvider extends BaseAIProvider {
  readonly providerName = ModelProvider.NVIDIA;
  readonly modelName: string;
  private client: OpenAI;

  constructor(apiKey: string, model = 'meta/llama-3.1-8b-instruct') {
    super();
    // NVIDIA NIM API is fully OpenAI-compatible. We just point the baseURL to NVIDIA.
  ...
  protected async _embed(request: EmbeddingRequest): Promise<EmbeddingResponse> {
    ...
    const response = await this.client.embeddings.create({
      // Fallback to NVIDIA's primary embedding model if none is provided
      model: request.model ?? 'nvidia/nv-embedqa-e5-v5',
      input,
